...
impl Client {
    ...
    pub(crate) async fn request_file(
        ...
        self.sender
            ...
            .await
            .expect("Command receiver not to be dropped.");
        Ok(receiver.await.expect("Sender not be dropped.")?)
    }
...
pub(crate) struct EventLoop {
    ...
    pending_request_file:
        HashMap<OutboundRequestId, oneshot::Sender<Result<Vec<u8>>>>,
}
...
impl EventLoop {
    ...
    fn dummy() -> Self {
        // This is a placeholder - it won't actually be used for anything
        // since we're replacing it with the real one
        unimplemented!("Dummy EventLoop should not be used")
    }
        ...
        match event {
            ...
            SwarmEvent::OutgoingConnectionError { peer_id, error, .. } => {
                if let Some(peer_id) = peer_id {
                    if let Some(sender) = self.pending_dial.remove(&peer_id) {
                        let _ = sender.send(Err(Box::new(error).into()));
                    }
                }
            }
        ...
        match command {
            Command::StartListening { addr, sender } => {
                let _ = match self.swarm.listen_on(addr) {
                    Ok(_) => sender.send(Ok(())),
                    Err(e) => sender.send(Err(Box::new(e).into())),
                };
            }
            Command::Dial {
                peer_id,
                ...
                if let hash_map::Entry::Vacant(e) = self.pending_dial.entry(peer_id) {
                    ...
                    match self.swarm.dial(peer_addr.with(Protocol::P2p(peer_id))) {
                        ...
                        Err(e) => {
                            let _ = sender.send(Err(Box::new(e).into()));
                        }
                    }
                } else {
